...
- 将见山重新设计为黑金仪式界面，加入局部暖光、轻量同心几何、10px 辅助信息、分段卡片显现、延迟印章和按键卡片历史。
- 启用 LVGL 压缩字体解码，并增加仓库门禁，修复“见山”页面数字正常刷新但中文为空的问题。
- “见山”真机排版改为与已确认预览一致的纸张式单屏布局：事实正文使用 18px 黑体，标题与短原文使用宋体，并缩小来源文字、取消强制断行，避免长卡溢出屏幕。

- 新增“见山”鼓劲卡 demo，首批包含三张注明出处的中文卡片；支持不连续重复的随机抽卡，并使用三个硬件按键浏览本次会话历史。
